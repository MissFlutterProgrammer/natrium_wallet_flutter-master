...
class MantaUtil {
  ...
  static Future<PaymentRequestMessage> getPaymentDetails(
      MantaWallet manta) async {
    await manta.connect();
    final RSAPublicKey cert = await manta.getCertificate();
    final PaymentRequestEnvelope payReqEnv =
        await manta.getPaymentRequest(cryptoCurrency: "NANO");
  ...
  static void processPaymentRequest(BuildContext context, MantaWallet manta,
    ...
    if (!Address(dest.destination_address).isValid()) {
      ...
      UIUtil.showSnackbar(
          AppLocalization.of(context).minimumSend.replaceAll("%1", "0.000001"),
          context);
    } else {
      // Is valid, proceed
